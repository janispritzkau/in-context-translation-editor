import { defaultWindow, useResizeObserver, type ConfigurableWindow } from "@vueuse/core";
...
export function useTextareaAutosize(options: UseTextareaAutosizeOptions = {}) {
  ...
  function triggerResize() {
    ...
    if (style.boxSizing === "content-box") {
      ...
      textareaScrollHeight.value =
        textarea.value?.scrollHeight + textarea.value?.offsetHeight - textarea.value?.clientHeight;
    }
  ...
  if (options?.watch) watch(options.watch, triggerResize, { immediate: true, deep: true });
